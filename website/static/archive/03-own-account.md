---
title : "3. [LEGACY] In Your Own Account"
weight : 3
---
To deploy the workshop infrastructure **outside of an AWS run event** you'll need to do the following:

### Deploy the AWS CloudFormation Stack

There are three modes for deploying the workshop CloudFormation stack.

- Full deployment mode: Creates all prerequisite resources (10 minutes)
- integrative deployment mode: Uses existing VPC and EKS cluster, but creates new network resources(subnet, security group, CIDR block), S3 bucket, and IAM role for the HyperPod cluster. (3 minutes)
- Minimal deployment: Uses exising VPC, network resources and EKS cluster. Creates only S3 bucket IAM role for the HyperPod cluster. (1 minute)

You can find detailed instructions in each tab below.
...
Click the button below, to deploy the CloudFormation stack. Select a deployment mode and enter the parameters. AvailabilityZoneId must be a valid Availability Zone ID for your region. The default value `usw2-az2` is valid only for `us-west-2` region.
...
Next make sure you have a linux based development environment. If you need a linux based development environment, you can launch a browser-based shell [AWS CloudShell](https://docs.aws.amazon.com/cloudshell/latest/userguide/welcome.html), from the AWS Management Console.
