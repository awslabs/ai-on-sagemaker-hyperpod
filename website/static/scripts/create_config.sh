...
if [ -z "${EKS_CLUSTER_NAME:-}" ]; then
    EKS_CLUSTER_NAME=$(get_cfn_output 'EKSClusterName')
fi
...
if [ -z "${EKS_CLUSTER_ARN:-}" ]; then
    EKS_CLUSTER_ARN=$(get_cfn_output 'EKSClusterArn')
...
BUCKET_NAME=$(get_cfn_output 'S3BucketName')
...
EXECUTION_ROLE=$(get_cfn_output 'SageMakerIAMRoleArn')
...
if [ -z "${SUBNET_ID:-}" ]; then
    SUBNET_ID=$(get_cfn_output 'PrivateSubnetId')
fi
...
if [ -z "${SECURITY_GROUP:-}" ]; then
    SECURITY_GROUP=$(get_cfn_output 'SecurityGroupId')
fi
