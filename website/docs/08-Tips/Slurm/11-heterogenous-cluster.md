...
If you wish to add a new worker-group with a new instance type to your cluster, we recommend you follow the steps provided here which use the [`update-cluster`](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-hyperpod-operate-cli-command.html#sagemaker-hyperpod-operate-cli-command-update-cluster) CLI command. When you update a cluster to create separate worker-groups for different instance types, this is called a *heterogenous cluster*. 
...
:::alert{header="Enable Optional Lifecycle Scripts" type="info"}
...
```
:::alert{header="update-cluster-config.json" type="info"}
...
aws sagemaker update-cluster \                
    ...
    --region $AWS_REGION
```
:::alert{header="update-cluster" type="info"}
...
```
:::alert{header="Default Partition" type="info"}
...
:::alert{header="Get S3 LifeCycle URI" type="info"}
...
```
:::alert{header="Verify slurm configuration" type="info"}
...
:::alert{header="Stopping Slurmctld" type="warning"}
...
```
:::alert{header="Stopping Slurmctld" type="info"}
