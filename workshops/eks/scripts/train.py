...
# Add numpy array reconstruction to safe globals for checkpoint loading
...
class DataTrainingArguments:
    ...
    do_preprocess: bool = field(
        default=True, metadata={"help": "Enable data preprocessing and tokenization"}
    )
    ...
    def __post_init__(self):
        ...
        if (
            ...
            raise ValueError(
                "Need either a dataset name or a training/validation file/folder."
            )
        else:
...
def main():
    ...
    if data_args.do_preprocess:
        tokenized_datasets = load_and_tokenize_data(
            raw_datasets, tokenizer, training_args, data_args
        )
    else:
    ...
    # Need a custom class due to these issues:
